    ...
    return jsonify(
        {
            ...
            "endpoint": "/predict",
            "metodo": "POST",
            "ejemplo_entrada": {"temperatura": 28},
        }
    )
...
def predict():
    ...
    if datos is None or "temperatura" not in datos:
        return (
            jsonify({"error": "Debes enviar un JSON con el campo 'temperatura'"}),
            400,
        )
    ...
    return jsonify(
        {"temperatura": temperatura, "prediccion_ventas": round(float(prediccion), 2)}
    )
